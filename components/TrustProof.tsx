  ...
  return (
    <section id="proof" className="relative scroll-mt-24 pb-10 pt-2 sm:pb-14" aria-labelledby="trust-heading">
      <div className="mx-auto max-w-wrap px-5 sm:px-8">
        ...
        <ul data-guide="proof-paths" className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {(t.raw("paths") as readonly { title: string; text: string }[]).map((path) => (
            <li key={path.title} className="gl-tile rounded-2xl px-5 py-5">
        ...
        <ul className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {facts.map((fact) => (
            <li key={fact.title} className="gl-tile rounded-2xl px-5 py-5">
